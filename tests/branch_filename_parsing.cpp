...
TEST_CASE("ParseManifestFileSuffix - branch format without timestamp",
          "[branch][parsing]")
{
    std::string_view branch_name;
    uint64_t term = 0;
    ...
    REQUIRE(
        eloqstore::ParseManifestFileSuffix("main_5", branch_name, term, ts));
    REQUIRE(branch_name == "main");
    REQUIRE(term == 5);
    ...
    REQUIRE(eloqstore::ParseManifestFileSuffix(
        "feature_10", branch_name, term, ts));
    REQUIRE(branch_name == "feature");
    REQUIRE(term == 10);
    REQUIRE_FALSE(ts.has_value());
    ...
    // Zero term
    REQUIRE(
        eloqstore::ParseManifestFileSuffix("main_0", branch_name, term, ts));
    REQUIRE(branch_name == "main");
    REQUIRE(term == 0);
    REQUIRE_FALSE(ts.has_value());
}

TEST_CASE("ParseManifestFileSuffix - branch format with timestamp",
          "[branch][parsing]")
{
    std::string_view branch_name;
    uint64_t term = 0;
    ...
    REQUIRE(eloqstore::ParseManifestFileSuffix(
        "main_5_123456", branch_name, term, ts));
    REQUIRE(branch_name == "main");
    REQUIRE(term == 5);
    ...
    // Different values
    REQUIRE(eloqstore::ParseManifestFileSuffix(
        "feature_10_789012", branch_name, term, ts));
    REQUIRE(branch_name == "feature");
    REQUIRE(term == 10);
    REQUIRE(ts.has_value());
    ...
    // Zero timestamp
    REQUIRE(
        eloqstore::ParseManifestFileSuffix("main_5_0", branch_name, term, ts));
    REQUIRE(ts.has_value());
    REQUIRE(ts.value() == 0);
}
...
TEST_CASE("ParseManifestFileSuffix - case normalization", "[branch][parsing]")
{
    ...
    // Mixed case in filename will be returned as-is
    REQUIRE(eloqstore::ParseManifestFileSuffix(
        "Feature_10_123", branch_name, term, ts));
    REQUIRE(branch_name == "Feature");  // Not normalized, returned as-is
}
...
TEST_CASE("ParseManifestFileSuffix - old format rejected", "[branch][parsing]")
{
    ...
    REQUIRE_FALSE(
        eloqstore::ParseManifestFileSuffix("5", branch_name, term, ts));
    REQUIRE_FALSE(
        eloqstore::ParseManifestFileSuffix("0", branch_name, term, ts));

    // Old archive format: term_timestamp (no branch)
    REQUIRE_FALSE(
        eloqstore::ParseManifestFileSuffix("5_123456", branch_name, term, ts));
}
...
TEST_CASE("ParseManifestFileSuffix - invalid formats", "[branch][parsing]")
{
    ...
    // Empty
    REQUIRE_FALSE(
        eloqstore::ParseManifestFileSuffix("", branch_name, term, ts));

    // Non-numeric term
    REQUIRE_FALSE(
        eloqstore::ParseManifestFileSuffix("main_abc", branch_name, term, ts));

    // Non-numeric timestamp
    REQUIRE_FALSE(eloqstore::ParseManifestFileSuffix(
        "main_5_abc", branch_name, term, ts));

    // Invalid branch name
    REQUIRE_FALSE(eloqstore::ParseManifestFileSuffix(
        "main.branch_5", branch_name, term, ts));

    // Missing components
    REQUIRE_FALSE(
        eloqstore::ParseManifestFileSuffix("main_", branch_name, term, ts));
    REQUIRE_FALSE(
        eloqstore::ParseManifestFileSuffix("_5", branch_name, term, ts));
}
...
TEST_CASE("BranchFileRange - sorting and comparison", "[branch][mapping]")
{
    ...
    REQUIRE(ranges[0].branch_name == "feature");
    REQUIRE(ranges[0].max_file_id == 50);
    REQUIRE(ranges[1].branch_name == "main");
    REQUIRE(ranges[1].max_file_id == 100);
    REQUIRE(ranges[2].branch_name == "hotfix");
    REQUIRE(ranges[2].max_file_id == 200);
}
...
TEST_CASE("BranchFileMapping - binary search lookup", "[branch][mapping]")
{
    ...
    auto it1 = eloqstore::FindBranchRange(mapping, 25);
    REQUIRE(it1 != mapping.end());
    REQUIRE(it1->branch_name == "feature");
    REQUIRE(it1->term == 3);

    auto it2 = eloqstore::FindBranchRange(mapping, 75);
    REQUIRE(it2 != mapping.end());
    REQUIRE(it2->branch_name == "main");

    auto it3 = eloqstore::FindBranchRange(mapping, 150);
    REQUIRE(it3 != mapping.end());
    REQUIRE(it3->branch_name == "hotfix");

    // Beyond max range
    auto it4 = eloqstore::FindBranchRange(mapping, 300);
...
TEST_CASE("BranchFileMapping - GetBranchNameAndTerm", "[branch][mapping]")
{
    ...
    REQUIRE(eloqstore::GetBranchNameAndTerm(mapping, 150, branch, term) ==
            true);
    REQUIRE(branch == "hotfix");
    REQUIRE(term == 1);

    REQUIRE(eloqstore::GetBranchNameAndTerm(mapping, 300, branch, term) ==
            false);  // Beyond range
}
...
TEST_CASE("BranchFileMapping - FileIdInBranch", "[branch][mapping]")
{
    ...
    // Test FileIdInBranch
    REQUIRE(eloqstore::FileIdInBranch(mapping, 25, "feature") == true);
    REQUIRE(eloqstore::FileIdInBranch(mapping, 25, "main") == false);
    REQUIRE(eloqstore::FileIdInBranch(mapping, 75, "main") == true);
    REQUIRE(eloqstore::FileIdInBranch(mapping, 75, "feature") == false);
    REQUIRE(eloqstore::FileIdInBranch(mapping, 150, "hotfix") == true);
    REQUIRE(eloqstore::FileIdInBranch(mapping, 300, "hotfix") ==
            false);  // Beyond range
}
...
TEST_CASE("BranchFileMapping - serialization roundtrip", "[branch][mapping]")
{
    ...
    REQUIRE(deserialized[0].branch_name == "main");
    REQUIRE(deserialized[0].term == 5);
    REQUIRE(deserialized[0].max_file_id == 100);
    REQUIRE(deserialized[1].branch_name == "feature");
    REQUIRE(deserialized[1].term == 3);
    REQUIRE(deserialized[1].max_file_id == 50);
    REQUIRE(deserialized[2].branch_name == "hotfix");
    REQUIRE(deserialized[2].term == 1);
    REQUIRE(deserialized[2].max_file_id == 200);
}
