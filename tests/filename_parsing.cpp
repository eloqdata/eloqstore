...
{
    // Test data files
    auto [type1, suffix1] = ParseFileName("data_123");
    REQUIRE(type1 == "data");
    REQUIRE(suffix1 == "123");

    auto [type2, suffix2] = ParseFileName("data_123_5");
    REQUIRE(type2 == "data");
    REQUIRE(suffix2 == "123_5");

    // Test manifest files
    auto [type3, suffix3] = ParseFileName("manifest");
    REQUIRE(type3 == "manifest");
    REQUIRE(suffix3 == "");

    auto [type4, suffix4] = ParseFileName("manifest_5");
    REQUIRE(type4 == "manifest");
    REQUIRE(suffix4 == "5");

    auto [type5, suffix5] = ParseFileName("manifest_5_123456789");
...
{
    // Empty string
    auto [type1, suffix1] = ParseFileName("");
    REQUIRE(type1 == "");
    REQUIRE(suffix1 == "");

    // No separator
    auto [type2, suffix2] = ParseFileName("data");
    REQUIRE(type2 == "data");
    REQUIRE(suffix2 == "");

    // Multiple separators
    auto [type3, suffix3] = ParseFileName("data_1_2_3");
    REQUIRE(type3 == "data");
...
TEST_CASE("ParseDataFileSuffix - legacy format rejected", "[filename]")
{
    ...
    FileId file_id = 0;
    uint64_t term = 0;
    REQUIRE_FALSE(ParseDataFileSuffix("123", file_id, term));
    REQUIRE_FALSE(ParseDataFileSuffix("0", file_id, term));
    REQUIRE_FALSE(ParseDataFileSuffix("999999", file_id, term));
}
...
TEST_CASE("ParseDataFileSuffix - term-aware format", "[filename]")
{
    ...
    FileId file_id = 0;
    uint64_t term = 0;
    REQUIRE(ParseDataFileSuffix("123_5", file_id, term));
    REQUIRE(file_id == 123);
    REQUIRE(term == 5);

    FileId file_id2 = 0;
    uint64_t term2 = 0;
    REQUIRE(ParseDataFileSuffix("0_1", file_id2, term2));
    REQUIRE(file_id2 == 0);
    REQUIRE(term2 == 1);

    FileId file_id3 = 0;
    uint64_t term3 = 0;
    REQUIRE(ParseDataFileSuffix("999_12345", file_id3, term3));
...
TEST_CASE("ParseDataFileSuffix - edge cases", "[filename]")
{
    ...
    FileId file_id1 = 0;
    uint64_t term1 = 0;
    REQUIRE_FALSE(ParseDataFileSuffix("", file_id1, term1));

    // Invalid format (non-numeric)
    FileId file_id2 = 0;
    uint64_t term2 = 0;
    REQUIRE_FALSE(ParseDataFileSuffix("abc", file_id2, term2));

    // Invalid format (non-numeric term)
    FileId file_id3 = 0;
    uint64_t term3 = 0;
    REQUIRE_FALSE(ParseDataFileSuffix("123_abc", file_id3, term3));

    // Invalid format (non-numeric file_id)
    FileId file_id4 = 0;
    uint64_t term4 = 0;
    REQUIRE_FALSE(ParseDataFileSuffix("abc_5", file_id4, term4));
}
...
TEST_CASE("ParseManifestFileSuffix - legacy format rejected", "[filename]")
{
    ...
    REQUIRE_FALSE(ParseManifestFileSuffix("", term, timestamp));
}
...
TEST_CASE("ParseManifestFileSuffix - term-only format", "[filename]")
{
    ...
    uint64_t term = 0;
    std::optional<uint64_t> timestamp;
    REQUIRE(ParseManifestFileSuffix("5", term, timestamp));
    REQUIRE(term == 5);
    REQUIRE(!timestamp.has_value());

    uint64_t term2 = 0;
    std::optional<uint64_t> timestamp2;
    REQUIRE(ParseManifestFileSuffix("0", term2, timestamp2));
    REQUIRE(term2 == 0);
    REQUIRE(!timestamp2.has_value());

    uint64_t term3 = 0;
    std::optional<uint64_t> timestamp3;
    REQUIRE(ParseManifestFileSuffix("12345", term3, timestamp3));
    REQUIRE(term3 == 12345);
...
TEST_CASE("ParseManifestFileSuffix - term-aware archive format", "[filename]")
{
    ...
    uint64_t term = 0;
    std::optional<uint64_t> timestamp;
    REQUIRE(ParseManifestFileSuffix("5_123456789", term, timestamp));
    REQUIRE(term == 5);
    REQUIRE(timestamp.has_value());
    ...
    uint64_t term2 = 0;
    std::optional<uint64_t> timestamp2;
    REQUIRE(ParseManifestFileSuffix("0_999999999", term2, timestamp2));
    REQUIRE(term2 == 0);
    REQUIRE(timestamp2.has_value());
    ...
    uint64_t term3 = 0;
    std::optional<uint64_t> timestamp3;
    REQUIRE(ParseManifestFileSuffix("123_456789012", term3, timestamp3));
    REQUIRE(term3 == 123);
    REQUIRE(timestamp3.has_value());
...
TEST_CASE("ParseManifestFileSuffix - edge cases", "[filename]")
{
    ...
    REQUIRE_FALSE(ParseManifestFileSuffix("abc", term1, timestamp1));

    // Invalid format (non-numeric term in archive)
    uint64_t term2 = 0;
    std::optional<uint64_t> timestamp2;
    REQUIRE_FALSE(ParseManifestFileSuffix("abc_123456789", term2, timestamp2));

    // Invalid format (non-numeric timestamp)
    uint64_t term3 = 0;
    std::optional<uint64_t> timestamp3;
    REQUIRE_FALSE(ParseManifestFileSuffix("5_abc", term3, timestamp3));
}

TEST_CASE("DataFileName - term-aware format", "[filename]")
{
    // Term-aware format: data_<id>_<term>
    std::string name1 = DataFileName(123, 5);
    REQUIRE(name1 == "data_123_5");

    std::string name2 = DataFileName(0, 1);
    REQUIRE(name2 == "data_0_1");

    std::string name3 = DataFileName(999, 12345);
    REQUIRE(name3 == "data_999_12345");

    // term=0 should also include explicit suffix
    std::string name4 = DataFileName(123, 0);
...
TEST_CASE("ManifestFileName - term-aware format", "[filename]")
{
    ...
    std::string name1 = ManifestFileName(5);
    REQUIRE(name1 == "manifest_5");

    std::string name2 = ManifestFileName(12345);
    REQUIRE(name2 == "manifest_12345");

    std::string name3 = ManifestFileName(1);
    REQUIRE(name3 == "manifest_1");

    // term=0 should also include explicit suffix
    std::string name4 = ManifestFileName(0);
...
TEST_CASE("ArchiveName - term-aware format", "[filename]")
{
    ...
    std::string name1 = ArchiveName(5, 123456789);
    REQUIRE(name1 == "manifest_5_123456789");

    std::string name2 = ArchiveName(0, 999999999);
    REQUIRE(name2 == "manifest_0_999999999");

    std::string name3 = ArchiveName(123, 456789012);
...
TEST_CASE("Roundtrip - DataFileName generate and parse", "[filename]")
{
    ...
    std::string name = DataFileName(123, 0);
    auto [type, suffix] = ParseFileName(name);
    REQUIRE(type == "data");
    FileId file_id = 0;
    uint64_t term = 0;
    REQUIRE(ParseDataFileSuffix(suffix, file_id, term));
    REQUIRE(file_id == 123);
    REQUIRE(term == 0);  // No term in legacy format

    // Test term-aware format roundtrip
    std::string name2 = DataFileName(456, 7);
    auto [type2, suffix2] = ParseFileName(name2);
    REQUIRE(type2 == "data");
    FileId file_id2 = 0;
    uint64_t term2 = 0;
    REQUIRE(ParseDataFileSuffix(suffix2, file_id2, term2));
...
TEST_CASE("Roundtrip - ManifestFileName generate and parse", "[filename]")
{
    ...
    std::string name = ManifestFileName(0);
    auto [type, suffix] = ParseFileName(name);
    REQUIRE(type == "manifest");
    uint64_t term = 0;
    std::optional<uint64_t> timestamp;
    REQUIRE(ParseManifestFileSuffix(suffix, term, timestamp));
    REQUIRE(term == 0);  // No term in legacy format
    REQUIRE(!timestamp.has_value());

    // Test term-aware format roundtrip
    std::string name2 = ManifestFileName(5);
    auto [type2, suffix2] = ParseFileName(name2);
    REQUIRE(type2 == "manifest");
    uint64_t term2 = 0;
    std::optional<uint64_t> timestamp2;
    REQUIRE(ParseManifestFileSuffix(suffix2, term2, timestamp2));
    REQUIRE(term2 == 5);
    REQUIRE(!timestamp2.has_value());
}
...
TEST_CASE("Roundtrip - ArchiveName generate and parse", "[filename]")
{
    ...
    std::string name = ArchiveName(5, 123456789);
    auto [type, suffix] = ParseFileName(name);
    REQUIRE(type == "manifest");
    uint64_t term = 0;
    std::optional<uint64_t> timestamp;
...
TEST_CASE("ParseUint64 - valid numbers", "[filename]")
{
    ...
    REQUIRE(ParseUint64("0", result));
    REQUIRE(result == 0);

    REQUIRE(ParseUint64("123", result));
    REQUIRE(result == 123);

    REQUIRE(ParseUint64("999999", result));
    REQUIRE(result == 999999);

    REQUIRE(ParseUint64("18446744073709551615", result));  // UINT64_MAX
...
TEST_CASE("ParseUint64 - invalid inputs", "[filename]")
{
    ...
    // Empty string
    REQUIRE(!ParseUint64("", result));

    // Non-numeric
    REQUIRE(!ParseUint64("abc", result));
    REQUIRE(!ParseUint64("123abc", result));
    REQUIRE(!ParseUint64("abc123", result));
...
TEST_CASE("Integration - complete filename workflow", "[filename]")
{
    ...
    REQUIRE(type == "data");
    FileId parsed_file_id = 0;
    uint64_t parsed_term = 0;
    REQUIRE(ParseDataFileSuffix(suffix, parsed_file_id, parsed_term));
    REQUIRE(parsed_file_id == file_id);
    REQUIRE(parsed_term == term);

    // Simulate creating a manifest with term
    uint64_t manifest_term = 7;
    std::string manifest_name = ManifestFileName(manifest_term);

    // Parse it back
    auto [manifest_type, manifest_suffix] = ParseFileName(manifest_name);
    REQUIRE(manifest_type == "manifest");
    uint64_t parsed_manifest_term = 0;
    std::optional<uint64_t> parsed_ts;
    REQUIRE(ParseManifestFileSuffix(manifest_suffix,
    ...
    std::string archive_name = ArchiveName(archive_term, timestamp);

    // Parse it back
    auto [archive_type, archive_suffix] = ParseFileName(archive_name);
    REQUIRE(archive_type == "manifest");
    uint64_t parsed_archive_term = 0;
    std::optional<uint64_t> parsed_archive_ts;
    REQUIRE(ParseManifestFileSuffix(archive_suffix,
                                    parsed_archive_term,
                                    parsed_archive_ts));
