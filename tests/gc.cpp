...
bool CheckLocalPartitionExists(const eloqstore::KvOptions &opts, const eloqstore::TableIdent &tbl_id)
{
...
bool CheckCloudPartitionExists(const eloqstore::KvOptions &opts, const eloqstore::TableIdent &tbl_id)
{
    ...
    std::vector<std::string> cloud_files = ListCloudFiles(
        opts.cloud_store_daemon_url,
        opts.cloud_store_path,
        tbl_id.ToString());
    
    LOG(INFO) << "CheckCloudPartitionExists, cloud_files size: " << cloud_files.size();
...
TEST_CASE("archive prevents data deletion after truncate", "[gc][archive]")
{
    ...
    if (!archive_gc_opts.cloud_store_path.empty())
    {
        ...
        std::vector<std::string> remaining_files = ListCloudFiles(
            archive_gc_opts.cloud_store_daemon_url,
    ...
    {
        // For local mode, check that partition directory still exists with archive
        REQUIRE(CheckLocalPartitionExists(archive_gc_opts, tbl_id));
        
        // Verify archive file exists in local directory
        fs::path partition_path = fs::path(archive_gc_opts.store_path[0]) / tbl_id.ToString();
