...
KvError CloudStoreMgr::DownloadFile(const TableIdent &tbl_id, FileId file_id)
{
    ...
    KvError status = download_task.error_;
    if (status != KvError::NoError)
    {
        return status;
    }

    status = WriteFile(tbl_id, filename, download_task.response_data_);
    if (status == KvError::NoError)
    {
                                 ...
                                 std::vector<std::string> &contents)
{
    ...
    for (size_t i = 0; i < filenames.size(); ++i)
    {
        ...
        else
        {
            ...
            path /= filename;
            KvError err = ReadFile(path.string(), contents[i]);
            if (err != KvError::NoError)
            {
                return err;
            }
...
KvError CloudStoreMgr::WriteFile(const std::string &file_path,
                                 std::string_view data)
{
    ...
    int fd = OpenAt({AT_FDCWD, false},
                    path_str.c_str(),
                    O_WRONLY | O_CREAT | O_TRUNC,
                    0644);
    if (fd < 0)
    {
        LOG(ERROR) << "failed to open file for write: " << file_path
                   << ", error=" << strerror(-fd);
        return ToKvError(fd);
    }
...
KvError MemStoreMgr::ReadFile(const std::string &file_path,
                              std::string &content)
{
    std::error_code ec;
    ...
    if (ec)
    {
        LOG(ERROR) << "Failed to determine file size: " << file_path << ": "
                   << ec.message();
        return KvError::IoFail;
    }
...
KvError MemStoreMgr::WriteFile(const std::string &file_path,
                               std::string_view data)
{
    __builtin_unreachable();
}
                             ...
                             std::string &content)
{
    ...
    FdIdx root_fd = GetRootFD(tbl_id);
    uint64_t flags = O_RDONLY;
    if (is_data_file)
    {
        flags |= O_DIRECT;
    }
    int fd = OpenAt(root_fd, rel_path_str.c_str(), flags);
    if (fd < 0)
    {
...
KvError IouringMgr::ReadArchiveFile(const std::string &file_path,
                                    std::string &content)
{
    return ReadFile(file_path, content);
}

KvError IouringMgr::ReadFile(const std::string &file_path, std::string &content)
{
    std::string path = file_path;
    int fd = OpenAt({AT_FDCWD, false}, path.c_str(), O_RDONLY);
    if (fd < 0)
    {
        LOG(ERROR) << "failed to open file: " << file_path
                   << ", error=" << strerror(-fd);
    ...
    else
    {
        ...
        {
            size_t to_read = file_size - off;
            int rres = Read({fd, false}, content.data() + off, to_read, off);
            if (rres < 0)
            {
                status = ToKvError(rres);
                break;
