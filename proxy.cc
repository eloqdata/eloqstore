...
static int setup_send_zc(struct io_uring *ring, struct conn *c)
{
	...
	buf = c->in_br.buf;
	iovs = (iovec *) calloc(nr_bufs, sizeof(struct iovec));
	for (i = 0; i < nr_bufs; i++) {
		iovs[i].iov_base = buf;
...
{
	const struct bucket_stat *b1 = (bucket_stat *) p1;
	const struct bucket_stat *b2 = (bucket_stat *) p2;
...
static void show_buckets(struct conn_dir *cd)
{
	...
	rstat = (bucket_stat*) calloc(nr_bufs + 1, sizeof(struct bucket_stat));
	sstat = (bucket_stat*) calloc(nr_bufs + 1, sizeof(struct bucket_stat));
...
static void init_mvec(struct msg_vec *mvec)
{
	...
	mvec->iov = (iovec *) malloc(sizeof(struct iovec));
	mvec->vec_size = 1;
}
...
static int handle_accept(struct io_uring *ring, struct io_uring_cqe *cqe)
{
	...
	for (i = 0; i < 2; i++) {
		...
		cd->rcv_next_bid = -1;
		if (ext_stat) {
			cd->rcv_bucket = (int *)calloc(nr_bufs + 1, sizeof(int));
			cd->snd_bucket = (int *)calloc(nr_bufs + 1, sizeof(int));
		}
		init_msgs(cd);
	}
...
static void send_append_vec(struct conn_dir *cd, void *data, int len)
{
	...
	if (mvec->iov_len == mvec->vec_size) {
		mvec->vec_size <<= 1;
		mvec->iov = (iovec *) realloc(mvec->iov, mvec->vec_size * sizeof(struct iovec));
	}
			  ...
			  int in_bytes)
{
	...
	while (in_bytes) {
		...
		pdu = (io_uring_recvmsg_out *) buf->addr;
