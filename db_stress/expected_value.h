...
{
public:
    static uint32_t GetValueBaseMask()
    {
        return VALUE_BASE_MASK;
    }
    static uint32_t GetValueBaseDelta()
    {
        return VALUE_BASE_DELTA;
    }
    static uint32_t GetDelCounterDelta()
    {
        return DEL_COUNTER_DELTA;
    }
    ...
    uint32_t GetValueBase() const
    {
        return GetValuePart(VALUE_BASE_MASK);
    }
    ...
    {
        ClearValuePart(PENDING_WRITE_MASK);
    }

    uint32_t GetDelCounter() const
    {
        return GetValuePart(DEL_COUNTER_MASK);
    }
    ...
    void SetPendingDel()
    {
        SetValuePart(PENDING_DEL_MASK, PENDING_DEL_MASK);
    }

    void ClearPendingDel()
    {
        ClearValuePart(PENDING_DEL_MASK);
    }
    ...
    void SetDeleted()
    {
        SetValuePart(DEL_MASK, DEL_MASK);
    }

    void ClearDeleted()
    {
        ClearValuePart(DEL_MASK);
    }
...
private:
                                     ...
                                     uint32_t value_part_delta) const
    {
        ...
        ExpectedValue temp_expected_value(current_value_part +
                                          value_part_delta);
...
{
public:
    explicit PendingExpectedValue(uint32_t *value,
    ...
    void Commit()
    {
        ...
        *value_ = final_value_.Read();
    }

    // Rollbacks the key to its original state.
    // This rollbacks the pending state created in `ExpectedState::Precommit`,
    // such as pending delete, pending put. If `ExpectedState::Precommit()` is
    // not called before creating this `PendingExpectedValue`, this is a no-op.
    void Rollback()
    {
        assert(!pending_state_closed_);
        ClosePendingState();
    ...
    uint32_t GetFinalValueBase()
    {
        return final_value_.GetValueBase();
    }

private:
    inline void ClosePendingState() const
    {
...
public:
    ...
    // `post_read_expected_value` is expected not to exist from begining till
    // the end of the read
    //
    ...
    // Return whether the key associated with `pre_read_expected_value` and
    // `post_read_expected_value` is expected to exist from begining till the
    // end of the read.
    //
    ...
    static bool InExpectedValueBaseRange(
        uint32_t value_base,
