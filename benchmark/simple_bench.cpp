...
void WriteLoop(eloqstore::EloqStore *store)
{
    ...
    if (recorded_write_latency)
    {
        ...
        const uint64_t mb_per_sec =
            ...
            20;
        const auto average =
            static_cast<uint64_t>(g_write_latency.latency());
        const auto p50 =
        ...
        const auto p9999 = static_cast<uint64_t>(
            g_write_latency.latency_percentile(0.9999));
        ...
        if (FLAGS_show_write_perf)
        {
            LOG(INFO) << "write summary " << kvs_per_sec << " kvs/s | cost "
                      ...
                      << " microseconds | p50 " << p50
                      << " microseconds | p90 " << p90
                      << " microseconds | p99 " << p99
...
void ReadLoop(eloqstore::EloqStore *store, uint32_t thd_id)
{
    ...
    while (true)
    {
        ...
        auto now = high_resolution_clock::now();
        double cost_ms =
            duration_cast<milliseconds>(now - last_log).count();
        ...
        const auto average =
            static_cast<uint64_t>(g_read_latency.latency());
        const auto p50 =
        ...
        const auto p9999 = static_cast<uint64_t>(
            g_read_latency.latency_percentile(0.9999));
        const auto max_latency =
...
void ScanLoop(eloqstore::EloqStore *store, uint32_t thd_id)
{
    ...
    while (true)
    {
        ...
        const auto average =
            static_cast<uint64_t>(g_scan_latency.latency());
        const auto p50 = static_cast<uint64_t>(
            g_scan_latency.latency_percentile(0.50));
        const auto p90 = static_cast<uint64_t>(
            g_scan_latency.latency_percentile(0.90));
        const auto p99 = static_cast<uint64_t>(
            g_scan_latency.latency_percentile(0.99));
        const auto p999 = static_cast<uint64_t>(
            g_scan_latency.latency_percentile(0.999));
        const auto p9999 = static_cast<uint64_t>(
            g_scan_latency.latency_percentile(0.9999));
        const auto max_latency =
