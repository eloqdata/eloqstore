...
int KvOptions::LoadFromIni(const char *path)
{
    ...
    if (reader.HasValue(sec_run, "max_concurrent_writes"))
    {
        max_concurrent_writes = reader.GetUnsigned(
            sec_run, "max_concurrent_writes", max_concurrent_writes);
    }
...
bool KvOptions::operator==(const KvOptions &other) const
{
    return num_threads == other.num_threads &&
           ...
           dict_cache_size == other.dict_cache_size &&
