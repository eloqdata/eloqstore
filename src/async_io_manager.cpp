                                        ...
                                        FileId file_id)
{
    ...
    if (fd_ref != nullptr)
    {
        ...
        uint64_t term = fd_ref.Get()->term_;
        KvError err =
            UploadFile(tbl_id, ToFilename(file_id, term), owner, {}, false);
        if (err == KvError::NoError)
        {
    ...
    uint64_t term = GetFileIdTerm(tbl_id, file_id).value_or(ProcessTerm());
    return UploadFile(
        tbl_id, ToFilename(file_id, term), owner, {}, owner == nullptr);
}
...
{
    KvTask *current_task = ThdTask();
    std::string upload_name = std::move(filename);
    std::unique_ptr<ObjectStore::UploadTask> pending_upload_task;
    ObjectStore::UploadTask stack_upload_task(&tbl_id, upload_name);
    ObjectStore::UploadTask *upload_task = &stack_upload_task;
    if (!wait_for_completion)
    {
        if (owner == nullptr)
        {
            LOG(ERROR)
                << "Async upload requires write-task owner, table=" << tbl_id
                << " filename=" << upload_name;
            return KvError::InvalidArgs;
        }
        pending_upload_task =
            std::make_unique<ObjectStore::UploadTask>(&tbl_id, upload_name);
        upload_task = pending_upload_task.get();
    }
    upload_task->SetKvTask(current_task);
    ...
    const bool release_before_wait = (owner != nullptr);
