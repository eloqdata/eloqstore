    ...
    uint64_t term)
{
    ...
    {
        // Check for term or branch_name mismatch in cloud mode.
        const bool cloud_mode = !options_->cloud_store_path.empty();
        if (cloud_mode && file_id != LruFD::kDirectory)
        {
            bool mismatch = false;
    ...
    const TableIdent &tbl_id)
{
    ...
    KvError term_err = UpsertTermFile(tbl_id, active_br, process_term);
...
KvError CloudStoreMgr::SyncFile(LruFD::Ref fd)
{
    ...
    if (file_id != LruFD::kDirectory)
    {
        ...
        err = UploadFile(tbl_id, filename, CurrentWriteTask(), {}, fd.FdPair());
        if (file_id == LruFD::kManifest)
        {
                   ...
                   err != KvError::OssInsufficientStorage)
            {
                ...
                err = UploadFile(
                    tbl_id, filename, CurrentWriteTask(), {}, fd.FdPair());
            }
        }
        if (err == KvError::NoError)
        {
    ...
    const TableIdent &tbl_id, std::vector<std::pair<std::string, FdIdx>> files)
{
    ...
    {
        KvError err =
            UploadFile(tbl_id, std::move(filename), owner, {}, cached_fd);
        if (err != KvError::NoError)
        {
            return err;
        }
